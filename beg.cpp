...
void beg(unordered_map<string, float>& variables, const string& givenVar) {
    if (!isalpha(givenVar[0])) {
        cout <<  "SNOL> Error: Unknown Word [" << givenVar << "]\n";
        return;
    }

	cout << "SNOL> Please enter value for [" << givenVar << "]\n";
    cout << "Input: ";
    float value;
    cin >> value;
    cin.ignore(numeric_limits<streamsize>::max(), '\n'); //Clears the input buffer. Loop through character from cin until a newline is encountered
    variables[givenVar] = value;
} //beg command BEG var, then ask input var>
...
int main() {
    ...
    while (true) {
        ...
        if (cmd == "HELP") {
            ...
            if(tokens.size() != 2) {
            	cout << "Error: BEG. Command requires a valid variable name";
			} else {
